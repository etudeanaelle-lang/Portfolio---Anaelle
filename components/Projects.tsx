      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        {PROJECTS.map((project, index) => (
          <div key={project.id} className={`group relative ${index % 2 !== 0 ? 'md:mt-24' : ''}`}>
            <div 
              className="overflow-hidden rounded-2xl bg-provence-lavender aspect-[4/3] relative cursor-pointer"
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-8">
                <span className="text-white font-medium px-4 py-2 border border-white/40 rounded-full text-sm backdrop-blur-sm">
                  Voir le projet
                </span>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-2">
              ...
              <h4 
                className="text-2xl font-serif text-provence-earth group-hover:text-provence-muted-purple transition-colors cursor-pointer"
                onClick={() => onProjectClick(project.id)}
              >
                {project.title}
              </h4>
